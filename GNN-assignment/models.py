...
class GraphSage(pyg_nn.MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, reducer='mean', 
        ...
        self.agg_lin = torch.nn.Linear(in_features=out_channels+in_channels,out_features=out_channels)
    ...
    def forward(self, x, edge_index):
        ...
        ############################################################################
        # Change the name to size1 to bypass the bug of pytorch geometric 1.6.1
        # See https://github.com/rusty1s/pytorch_geometric/issues/1760
        return self.propagate(edge_index, size1=(num_nodes,num_nodes), x=out)

    def message(self, x_j, edge_index, size1):
        # x_j has shape [E, out_channels]

        row, col = edge_index
        deg = pyg_utils.degree(row, size1[0], dtype=x_j.dtype)
    ...
    def update(self, aggr_out,x):
        ...
        if self.normalize_emb:
            aggr_out = F.layer_norm(aggr_out,(self.agg_lin.out_features,))
...
class GAT(pyg_nn.MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, num_heads=1, concat=True,
        ...
        self.lin = nn.Linear(in_features=in_channels,out_features=out_channels) # TODO
    ...
    def message(self, edge_index_i, x_i, x_j, size_i):
        ...
        #return (x_j * alpha.unsqueeze(-1)).view()
        result =  (x_j * alpha.unsqueeze(-1)).view(-1, self.heads * self.out_channels)
        return result
